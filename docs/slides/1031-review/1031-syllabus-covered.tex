...
\item[\done]  Design combinational circuits for positive and negative logic
  \it[\done]  Design Hazard-free two level circuits.
  \it[\done] Compute fan out and noise margin of one device driving the same time
  \it[\done] Compute noise margin of one device
  \it[\done] Describe how tri-state and open-collector outputs are different from totem-pole outputs.
  \it[\done] Different between and limitations of level-triggered latches and edge-triggered flip-flops.
  \it[\done] Know the differences and similarities between PAL, PLA, and ROMs and can use each for logic design
  \it[\done] Design combinational circuits using multiplexers and decoders
\end{todolist}
